...
public class AvatarReporter implements IAvatarReporter {
    ...
    @Override
    public Single<Status> updateReported(IDeviceSideAvatar deviceSideAvatar, Map<PropertyKey, PropertyValue> avatarProperties) {
        if (!rateLimiters.computeIfAbsent(deviceSideAvatar.getAvatarId(),
                k -> RateLimiter.create(avatarMaxReportTimePerSecond)).tryAcquire()) {
            log.debug("Avatar report too frequently, waiting for next slot, avatarId={}", deviceSideAvatar.getAvatarId());
            return waitingReport(deviceSideAvatar, avatarProperties);
        }
        if (reportWaitingRunning.get() || !waitingReportQueue.isEmpty()) {
            log.debug("There are too many inProcess messages, waiting for next slot, avatarId={}", deviceSideAvatar.getAvatarId());
            return waitingReport(deviceSideAvatar, avatarProperties);
        }
    ...
    static class WaitingReportQueue {
        ...
        void put(IDeviceSideAvatar deviceSideAvatar, Map<PropertyKey, PropertyValue> properties,
            ...
            WaitingReport waitingReport = waitingQueue.computeIfAbsent(
                    deviceSideAvatar.getAvatarId(), k -> new WaitingReport(deviceSideAvatar.getAvatarId(), deviceSideAvatar));
            ...
            properties.forEach((key, value) -> {
                ...
                if (log.isDebugEnabled() && oldValue != null) {
                    log.debug("The old value {} of key {} will be dropped since the new value {} will be send, avatarId={}",
