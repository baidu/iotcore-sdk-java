...
public class ReactiveStreamsMqttClientWrapper {
    ...
    public Single<Observable<UserMessage>> subscribe(Topic topic) {
                ...
                client.subscribe(topic.getTopicFilter(), topic.getQos(), null, new IMqttActionListener() {
                    ...
                    public void onSuccess(IMqttToken asyncActionToken) {
                        if (asyncActionToken.getGrantedQos()[0] == 1) {
                            log.debug("Mqtt client subscribe success, entityId={}, topic={}",
                                    config.getEntityId(), topic);
                            ...
                            emitter.onError(new RuntimeException("Get getGrantedQos "
                    ...
                    public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
                        log.error("Mqtt client subscribe error, entityId={}, topic={}",
                                config.getEntityId(), topic, exception);
        ...
        }).timeout(config.getSubscribeTimeoutMs(), TimeUnit.MILLISECONDS,
                AvatarSchedulers.io(config.getEntityId().getIotCoreId()));
    }
    ...
    public Completable publish(Topic topic, UserMessage userMessage) {
        return Completable.create(
                emitter -> {
                    try {
                        client.publish(topic.getTopicFilter(), userMessage.toMqttMessage(),
                ...
                .timeout(config.getPublishTimeoutMs(), TimeUnit.MILLISECONDS,
                        AvatarSchedulers.io(config.getEntityId().getIotCoreId()));
    }
