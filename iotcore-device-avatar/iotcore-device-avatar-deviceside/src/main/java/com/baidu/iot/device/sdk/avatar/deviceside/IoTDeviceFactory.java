...
public class IoTDeviceFactory {
    ...
    public IoTDeviceFactory(Config config) {
        ...
        this.avatarController = new AvatarController(config.iotCoreId,
                config.avatarMinReloadIntervalInMs, config.controlQueueCrowdedMaxDurationInMs);
        this.inProcessMessageQueue = new InProcessMessageQueue(config.iotCoreId,
                config.maxInProcessMessageCount, config.inProcessMessageExpireTimeMs);
    }
    ...
    public Single<Device> getDevice(String deviceName, MqttConfig mqttConfig) {
        return registeredDevices.computeIfAbsent(deviceName, k -> {
            IAvatarConsistenceStrategy consistenceStrategy =
                    new AvatarConsistenceStrategy(config.avatarActiveTimeWatershedInMs);
            ...
            return avatarTransport.map(transport -> {
                IDeviceSideAvatar deviceSideAvatar = new DeviceSideAvatar(
                        avatarId, consistenceStrategy, transport,
