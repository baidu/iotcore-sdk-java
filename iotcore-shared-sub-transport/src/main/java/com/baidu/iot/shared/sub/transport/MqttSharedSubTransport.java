...
public class MqttSharedSubTransport implements ISharedSubTransport {
    ...
    @VisibleForTesting
    final static long retryIntervalInMs = 5000;
    @VisibleForTesting
    final static String shareTopicPrefix = "$share/";
    @VisibleForTesting
    final static String oshareTopicPrefix = "$oshare/";
    ...
    public static ISharedSubTransport create(MqttTransportConfig config,
        ...
        for (String topicFilter : config.getTopicFilters()) {
            if (!topicFilter.startsWith(shareTopicPrefix) && !topicFilter.startsWith(oshareTopicPrefix)) {
                topicFilter = shareTopicPrefix + config.getTransportId().getGroupKey() + "/" + topicFilter;
            }
     ...
     * Connection and subs would retry{@link #retryTimes} when failed.
     *
     * @return
     */
    @Override
            ...
            List<Single<Boolean>> initResults = clientPool.stream().map(client ->
                    Single.<Boolean>create(emitter -> {
                        Completable connWithRetry = Completable.defer(() -> internalConnect(client))
                                ...
                                .retryWhen(new RetryWithDelay(retryTimes, retryIntervalInMs, scheduler));
    ...
    private Disposable recoverySubscribe(IMqttAsyncClient client) {
        return Completable.defer(() -> internalSubscribe(client))
                .subscribeOn(scheduler)
                .retryWhen(new RetryWithDelay(Integer.MAX_VALUE, retryIntervalInMs, scheduler))
